<template>
  <div class="page">
    <van-nav-bar title="我的" :right-text="showText" @click-right="changeStatus()"></van-nav-bar>
    <div class="content">
        <div class="person">
          <div class="icon"></div>
        ...
        <van-grid :column-num="4" class="orders">
          <van-grid-item icon="balance-pay" text="待付款" :badge="status1Item" flag=1 />
          <van-grid-item icon="completed" text="已付款" :badge="status2Item" flag=2 />
          <van-grid-item icon="failure" text="已取消" :badge="status0Item" flag=0 />
          <van-grid-item icon="description" text="全部订单" :badge="uid!=''? orderList.length:0" flag=3 />
        </van-grid>
        <van-divider :style="{ color: '#1989fa', borderColor: '#1989fa', padding: '0 16px' }">个人信息</van-divider>
        ...
        <van-popup
          ...
          :style="{ height: '50%' }"
        >
            <van-divider :style="{ color: '#1989fa', borderColor: '#1989fa', padding: '0 16px' }">修改密码</van-divider>
            <p style="float:left; text-indent:1em;">用户id：{{uid}}</p>
            <van-field v-model="newPsw" type="password" placeholder="请输入新密码(最短6位)" label="密码" />
            <van-button type="danger" :disabled="newPsw.length<6" @click="toUpDatePsw()">确认修改</van-button>
        </van-popup>
    </div>
  </div>
</template>
