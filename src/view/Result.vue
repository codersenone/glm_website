<template>
  <div class="page">
    ...
    <div class="content">
      ...
      <div v-if="addrList.length != 0">
        <select v-model="selectAddr" class="selectAddr">
          <option v-for="item in addrList" :value="item.aid">
            姓名：{{ item.name }},联系方式：{{ item.phone }} ,详细地址：{{
              item.addr
            }}
          </option>
        </select>
      ...
      <div v-else>
        你还没有创建地址信息<button class="toAddAddress" @click="toAddAddress">
          去创建地址信息
        </button>
      </div>
      <ul class="showList">
        <li
          class="li_normal mui-table-view-cell mui-media"
    ...
    <van-overlay :show="show">
      <div class="wrapper">
        <div class="block">
          正在提交订单<van-loading style="margin-top: 50px" />
        </div>
      </div>
    </van-overlay>
  </div>
...
export default {
  ...
  created() {
    ...
    // 获取用户地址信息
    if (this.uid != "") {
      request("/getAddress?uid=" + this.uid).then((res) => {
        let data = res.data;
        this.addrList = data;
      });
    }
    ...
    toPost(event) {
      ...
      if (this.selectAddr == null) {
        ...
        function getOId() {
          // 生成订单id
          let oid =
            Date.now().toString(10).slice(5) + parseInt(Math.random() * 100000);
          return oid;
        }
        this.oid = getOId();
        // 查询用户需要的商品是否足够库存
        request({
          ...
          if (unEnough.length != 0) {
            this.show = false;
            this.$dialog.alert({
              message: "商品" + unEnough[0] + "库存不足!请重新选择商品",
            });
          } else {
            // 购买商品以后修改商品库存
            request({
